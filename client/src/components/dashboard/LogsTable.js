...
export default function LogsTable(props) {
  ...
  const getData = async () => {
    try {
      await axios.get(`https://practo-backend-ib8w.onrender.com/api/exams/examsByProf?exam_code=${exam_code}&prof_email=${props.prof_email}`);
    } catch (err) {
    ...
    try {
      const response = await axios.post(`https://practo-backend-ib8w.onrender.com/api/logs/allData`, { exam_code });
      setErrorText('');
      setVisibility(true);
